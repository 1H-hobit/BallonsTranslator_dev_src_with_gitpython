...
class DrawingPanel(Widget):
    ...
    def runInpaint(self, inpaint_dict=None):
        ...
        if inpaint_dict is None:
            ...
            # 将笔划遮罩放置到正确位置
            mask_h, mask_w = mask.shape[:2]
            mask_x, mask_y = int(rect[0]), int(rect[1])
            mask_x2, mask_y2 = min(mask_x + mask_w, im_w), min(mask_y + mask_h, im_h)
            
            # 确保坐标在图像范围内
            mask_x = max(0, mask_x)
            mask_y = max(0, mask_y)
            mask_x2 = min(im_w, mask_x2)
            mask_y2 = min(im_h, mask_y2)
            
            # 计算实际可放置的区域大小
            actual_mask_w = mask_x2 - mask_x
            actual_mask_h = mask_y2 - mask_y
            
            # 将笔划遮罩复制到全图遮罩的相应位置
            if actual_mask_w > 0 and actual_mask_h > 0:
                full_mask[mask_y:mask_y2, mask_x:mask_x2] = mask[:actual_mask_h, :actual_mask_w]
            
            # 创建全图修复字典
            inpaint_rect = [0, 0, im_w, im_h]
            inpaint_dict = {'img': img, 'mask': full_mask, 'inpaint_rect': inpaint_rect}
            inpaint_dict['full_image'] = True  # 标记为全图修复
    ...
    def on_inpaint_finished(self, inpaint_dict):
        ...
        # 全图修复不需要合并遮罩
        if inpaint_dict.get('full_image', False):
            # 对于全图修复，直接使用整个修复后的图像
